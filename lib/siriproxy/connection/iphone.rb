...
class SiriProxy::Connection::Iphone < SiriProxy::Connection
  ...
  def post_init
    super
    start_tls(:cert_chain_file  => File.expand_path("#{SiriProxy::CONFIG_DIR}/server.passless.crt"),
              :private_key_file => File.expand_path("#{SiriProxy::CONFIG_DIR}/server.passless.key"),
              :verify_peer      => false)
  end
